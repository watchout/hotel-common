...
export class DeepLTranslationService {
  ...
  /** 言語コード変換（DeepL形式） */
  private mapToDeepLLanguage(language: SupportedLanguage): string {
  ...
  async translateBatch(
    ...
    sourceLanguage: SupportedLanguage = 'ja'
  ...
  /** 繁体字特化処理 */
  private async applyTraditionalChineseConversion(
    text: string
  ): Promise<string> {
    // 簡体字→繁体字変換ロジック
  ...
  /** 韓国語敬語チェック */
  private hasKoreanHonorific(text: string): boolean {
    // 簡易敬語パターンチェック
