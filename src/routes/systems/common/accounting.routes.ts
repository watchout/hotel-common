...
const PaymentRecordSchema = z.object({
  ...
  payment_method: z.enum(['cash', 'credit_card', 'bank_transfer', 'digital_wallet']),
...
router.get('/invoices', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const [totalCount, invoices] = await Promise.all([
      hotelDb.getAdapter().invoice.count({ where: whereClause }),
      hotelDb.getAdapter().invoice.findMany({
        where: whereClause,
    ...
    const [statusCounts] = await Promise.all([
      hotelDb.getAdapter().invoice.groupBy({
        by: ['status'],
...
router.get('/invoices/:id', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // データベースから請求書詳細を取得
    const invoice = await hotelDb.getAdapter().invoice.findFirst({
      where: {
        id,
        tenantId,
        isDeleted: false
    ...
    if (!invoice) {
      ResponseHelper.sendNotFound(res, '指定された請求書が見つかりません')
      return
    }
    ...
    const invoiceDetail = {
      ...
      payments: invoice.payments.map(payment => ({
        ...
        method: (payment.metadata as any)?.method || 'unknown',
    ...
    ResponseHelper.sendSuccess(res, { invoice: invoiceDetail })
...
router.get('/payments', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const [totalCount, payments] = await Promise.all([
      hotelDb.getAdapter().payment.count({ where: whereClause }),
      hotelDb.getAdapter().payment.findMany({
        where: whereClause,
        include: {
          invoice: true
        },
...
router.get('/reports', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const [invoiceSummary, paymentSummary, invoiceStatusCounts, paymentMethodCounts] = await Promise.all([
      hotelDb.getAdapter().invoice.aggregate({
        where: {
          tenantId,
          isDeleted: false,
      ...
      hotelDb.getAdapter().payment.aggregate({
        where: {
          tenantId,
          isDeleted: false,
    ...
    ResponseHelper.sendSuccess(res, { report })
