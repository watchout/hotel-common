/**
 * Prismaスキーマに@@mapディレクティブを追加するスクリプト（修正版）
 * 
 * 使用方法:
 * node scripts/add-mapping-directives-fixed.js [--dry-run] [--models=model1,model2]
 * 
 * オプション:
...
for (const model of models) {
  ...
  // 元のモデル定義
  const originalModelDef = model.fullMatch;
  
  // 新しいモデル定義（閉じ括弧の前にマッピングディレクティブを挿入）
  const newModelDef = originalModelDef.replace(
    /}$/,
    `\n\n  @@map("${tableName}")\n}`
  );
  
  // スキーマ内のモデル定義を置換
  const startPos = model.startIndex + offset;
  const endPos = startPos + originalModelDef.length;
  
  newSchemaContent = 
    newSchemaContent.substring(0, startPos) + 
    newModelDef + 
    newSchemaContent.substring(endPos);
  
  // オフセットを更新
  offset += newModelDef.length - originalModelDef.length;
