...
const AccountingQuerySchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['invoice', 'payment', 'refund']).optional(),
    status: zod_1.z.enum(['pending', 'completed', 'cancelled']).optional(),
    start_date: zod_1.z.string().optional(),
    end_date: zod_1.z.string().optional(),
    guest_id: zod_1.z.string().optional(),
    room_number: zod_1.z.string().optional()
});
/**
 * 決済処理スキーマ
 */
const PaymentProcessSchema = zod_1.z.object({
    invoice_id: zod_1.z.string(),
    payment_method: zod_1.z.enum(['cash', 'credit_card', 'debit_card', 'bank_transfer', 'digital_wallet']),
 ...
 */
router.get('/accounting', middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const [totalCount, transactions] = await Promise.all([
            database_1.hotelDb.getAdapter().transaction.count({ where: whereClause }),
            database_1.hotelDb.getAdapter().transaction.findMany({
                where: whereClause,
                include: {
                    invoice: true,
                    payment: true
        ...
        const filteredTransactions = transactions.map(transaction => ({
            ...
            room_number: transaction.metadata?.roomNumber || '不明',
        ...
        const [statusCounts, paymentMethodCounts, revenueSums] = await Promise.all([
            database_1.hotelDb.getAdapter().transaction.groupBy({
                by: ['status'],
            ...
            database_1.hotelDb.getAdapter().transaction.aggregate({
                where: { tenantId, isDeleted: false },
                _sum: {
                    totalAmount: true
                }
        ...
        logger.error('フロントデスク会計取引一覧取得エラー', error);
        if (error instanceof zod_1.z.ZodError) {
            api_response_standards_1.ResponseHelper.sendValidationError(res, 'クエリパラメータが正しくありません', error.errors);
            return;
        }
        api_response_standards_1.ResponseHelper.sendInternalError(res, '会計取引一覧の取得に失敗しました');
    }
});
/**
 * フロントデスク - 会計取引詳細取得
 ...
 */
router.get('/accounting/:id', middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        // データベースから取引詳細を取得
        const transaction = await database_1.hotelDb.getAdapter().transaction.findFirst({
            where: {
                id: transactionId,
                tenantId,
                isDeleted: false
        ...
        if (!transaction) {
            api_response_standards_1.ResponseHelper.sendNotFound(res, '指定された取引が見つかりません');
            return;
        }
        // レスポンス形式に変換
        const transactionDetail = {
            ...
            room_number: transaction.metadata?.roomNumber || '不明',
            check_in: transaction.metadata?.checkIn ? new Date(transaction.metadata.checkIn) : null,
            check_out: transaction.metadata?.checkOut ? new Date(transaction.metadata.checkOut) : null,
            amount: transaction.amount,
            tax_amount: transaction.taxAmount,
            total_amount: transaction.totalAmount,
            status: transaction.status,
        ...
        api_response_standards_1.ResponseHelper.sendSuccess(res, { transaction: transactionDetail });
        ...
        logger.error('フロントデスク会計取引詳細取得エラー', error);
        api_response_standards_1.ResponseHelper.sendInternalError(res, '会計取引詳細の取得に失敗しました');
    }
});
/**
 * フロントデスク - 決済処理
 ...
 */
router.post('/accounting/process-payment', middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        logger.error('フロントデスク決済処理エラー', error);
        if (error instanceof zod_1.z.ZodError) {
            api_response_standards_1.ResponseHelper.sendValidationError(res, '決済データが正しくありません', error.errors);
            return;
        }
        api_response_standards_1.ResponseHelper.sendInternalError(res, '決済処理に失敗しました');
    }
});
/**
 * フロントデスク - 日次売上レポート
 ...
 */
router.get('/accounting/daily-report', middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        api_response_standards_1.ResponseHelper.sendSuccess(res, { report: dailyReport });
        ...
        logger.error('フロントデスク日次売上レポート取得エラー', error);
        api_response_standards_1.ResponseHelper.sendInternalError(res, '日次売上レポートの取得に失敗しました');
    }
});
