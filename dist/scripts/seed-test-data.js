 ...
 */
async function confirmDatabaseOperation(message) {
...
async function seedTestData() {
    ...
    // データベース操作の確認
    const confirmed = await confirmDatabaseOperation('このスクリプトはPrismaを使用してデータベースにテストデータを挿入します。\n' +
    ...
    try {
        ...
        try {
            ...
            if (staffCount === 0) {
                // 直接SQLを実行する前に確認
                const sqlConfirmed = await confirmDatabaseOperation('スタッフテーブルへの直接SQLによるデータ挿入を行います。\n' +
            ...
            VALUES 
              (${(0, uuid_1.v4)()}, ${defaultTenant.id}, 'admin@example.com', ${await bcrypt.hash('admin123', 10)}, 'admin', '管理者', ${new Date()}, ${new Date()}),
              (${(0, uuid_1.v4)()}, ${defaultTenant.id}, 'staff@example.com', ${await bcrypt.hash('staff123', 10)}, 'staff', 'スタッフ', ${new Date()}, ${new Date()}),
              (${(0, uuid_1.v4)()}, ${testTenant.id}, 'test@example.com', ${await bcrypt.hash('test123', 10)}, 'admin', 'テスト管理者', ${new Date()}, ${new Date()})
          `;
