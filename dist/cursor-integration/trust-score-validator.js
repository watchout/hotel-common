...
exports.TrustScoreCalculator = exports.KnowledgeBaseValidator = void 0;
...
class KnowledgeBaseValidator {
    ...
    async initialize() {
        try {
            // Prismaスキーマ読み込み
            this.prismaSchema = await (0, promises_1.readFile)('prisma/schema.prisma', 'utf-8');
            // API仕様読み込み（存在する場合）
            try {
                const apiSpecFiles = [
     ...
     * Prismaスキーマとの照合検証
     */
    async verifyAgainstPrismaSchema(content) {
        const results = [];
        // テーブル名検証
     ...
     * TypeScript型安全性検証
     */
    async verifyTypeScriptSafety(content) {
        const results = [];
        // "as any" 使用検出
     ...
     * ビジネスロジック検証
     */
    async verifyBusinessLogic(content) {
        const results = [];
        // tenant_id必須チェック
...
exports.KnowledgeBaseValidator = KnowledgeBaseValidator;
/**
 * 信頼スコア計算エンジン
 */
class TrustScoreCalculator {
    ...
    async calculateTrustScore(content, context = {}) {
        ...
        // 重大な違反による減点
        const criticalFailures = factChecks.filter(fc => !fc.verified &&
            (fc.category === 'business_logic' || fc.item.includes('tenant_id'))).length;
