...
class HotelEventPublisher {
    ...
    async publishRealtimeEvent(event) {
        ...
        try {
            ...
            this.logger.error('リアルタイムイベント発行エラー:', error);
            throw error;
        }
    }
    /**
    ...
    async scheduleBatchEvent(event) {
        try {
            ...
            this.logger.error('バッチイベントスケジューリングエラー:', error);
            throw error;
        }
    }
    /**
     * WebSocketブロードキャスト
     */
    async broadcastWebSocketEvent(event) {
        try {
            if (!this.webSocketClient)
                return;
            // 対象システムに即座配信
            ...
            this.logger.warn('WebSocketブロードキャストエラー（継続）:', error);
     ...
     * イベント検証・強化
     */
    async validateAndEnrichEvent(event) {
        // 基本検証
