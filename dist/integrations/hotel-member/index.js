...
🔐 セキュリティ: 階層別データアクセス制御
    `);
        ...
        logger.error('hotel-member階層権限管理統合初期化エラー:', error);
        throw error;
    }
}
/**
...
export class HotelMemberHierarchyUtils {
     ...
     * グループ管理者かどうかチェック
     */
    static async isGroupManager(token) {
        try {
     ...
     * 店舗レベル以下かどうかチェック
     */
    static async isHotelLevelOrBelow(token) {
        try {
            const level = await this.getUserHierarchyLevel(token);
...
export const PYTHON_MIDDLEWARE_HELPERS = {
     ...
     */
    createAuthDependencyResponse: (user) => ({
     ...
     */
    createErrorResponse: (status_code, detail, error_type) => ({
        status_code,
        detail,
     ...
     */
    createPermissionResponse: (allowed, reason, metadata) => ({
        allowed,
        reason,
        metadata,
        timestamp: new Date().toISOString()
