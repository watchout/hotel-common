...
exports.SessionMigrationService = void 0;
...
class SessionMigrationService {
     ...
     * 既存注文データのセッション紐付け処理
     */
    static async migrateExistingOrders(tenantId) {
        const errors = [];
        let migratedCount = 0;
        try {
            logger_1.logger.info('既存注文データのセッション紐付け開始', { tenantId });
            // セッションIDが未設定の注文を取得
            const unmappedOrders = await prisma_1.hotelDb.getAdapter().order.findMany({
                where: {
                    tenantId,
                    sessionId: null,
                    isDeleted: false
            ...
            logger_1.logger.info('未紐付け注文数', { count: unmappedOrders.length, tenantId });
            for (const order of unmappedOrders) {
                try {
                    ...
                    if (session) {
                        // 注文にセッションIDを設定
                        await prisma_1.hotelDb.getAdapter().order.update({
                            where: { id: order.id },
                            data: {
                        ...
                        migratedCount++;
                        logger_1.logger.info('注文セッション紐付け完了', {
                            orderId: order.id,
                            sessionId: session.id,
                        ...
                        if (fallbackSession) {
                            ...
                            migratedCount++;
                            logger_1.logger.info('フォールバックセッション作成・紐付け完了', {
                                orderId: order.id,
                                sessionId: fallbackSession.id,
                    ...
                    logger_1.logger.error('注文セッション紐付けエラー', { orderId: order.id, error });
                }
            }
            logger_1.logger.info('既存注文データのセッション紐付け完了', {
                tenantId,
            ...
            logger_1.logger.error('セッション移行処理エラー', { tenantId, error });
            return {
                success: false,
                migratedCount,
    ...
    static async findSessionForOrder(tenantId, order) {
        ...
        const nearestSession = await prisma_1.hotelDb.getAdapter().checkinSession.findFirst({
            where: {
                tenantId,
                roomId: order.roomId
            },
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    static async createFallbackSession(tenantId, order) {
        try {
            ...
            if (!room) {
                logger_1.logger.error('部屋が見つかりません', { roomId: order.roomId });
                return null;
            }
            // 注文時刻に基づく予約を検索
            ...
            const fallbackReservation = reservation || await prisma_1.hotelDb.getAdapter().reservation.findFirst({
                where: {
                    tenantId,
                    roomId: order.roomId
                },
            ...
            if (!fallbackReservation) {
                logger_1.logger.error('対応する予約が見つかりません', { roomId: order.roomId });
                return null;
            }
            // セッション番号生成
            ...
            // フォールバックセッション作成
            const fallbackSession = await prisma_1.hotelDb.getAdapter().checkinSession.create({
                data: {
                    tenantId,
                    sessionNumber,
            ...
            logger_1.logger.info('フォールバックセッション作成完了', {
                sessionId: fallbackSession.id,
                sessionNumber: fallbackSession.sessionNumber,
                orderId: order.id,
                roomId: order.roomId
            ...
            logger_1.logger.error('フォールバックセッション作成エラー', { orderId: order.id, error });
            return null;
        }
    }
    /**
     * セッション統計情報取得
     */
    static async getSessionStatistics(tenantId) {
        try {
            const [totalSessions, activeSessions, totalOrders, mappedOrders, unmappedOrders] = await Promise.all([
                prisma_1.hotelDb.getAdapter().checkinSession.count({
                    where: { tenantId }
                }),
                prisma_1.hotelDb.getAdapter().checkinSession.count({
                    where: { tenantId, status: 'ACTIVE' }
                }),
                prisma_1.hotelDb.getAdapter().order.count({
            ...
            logger_1.logger.error('セッション統計取得エラー', { tenantId, error });
            throw error;
        }
    }
    /**
     * 後方互換性チェック
     */
    static async checkBackwardCompatibility(tenantId) {
        try {
            // セッションIDなしでも動作する既存APIの確認
            const compatibilityChecks = {
                ordersWithoutSession: await prisma_1.hotelDb.getAdapter().order.count({
                    where: { tenantId, sessionId: null, isDeleted: false }
                }),
                reservationsWithoutSession: await prisma_1.hotelDb.getAdapter().reservation.count({
                    where: {
                        tenantId,
                        sessions: { none: {} }
                    }
                }),
                activeReservationsNeedingSessions: await prisma_1.hotelDb.getAdapter().reservation.count({
                    where: {
                        tenantId,
                        status: 'checked_in',
                        sessions: { none: {} }
            ...
            logger_1.logger.error('後方互換性チェックエラー', { tenantId, error });
            throw error;
        }
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
exports.SessionMigrationService = SessionMigrationService;
exports.default = SessionMigrationService;
