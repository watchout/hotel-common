 ...
 * 複数システム共通の基幹データ管理
 */
class RoomGradeService {
    static logger = logger_1.HotelLogger.getInstance();
    /**
     * 客室ランク作成
     */
    static async createRoomGrade(data) {
        try {
            ...
            const existingGrade = await prisma_1.hotelDb.getAdapter().room_grades.findFirst({
                where: {
                    tenant_id: data.tenant_id,
                    name: data.name
                }
            ...
            const roomGrade = await prisma_1.hotelDb.getAdapter().room_grades.create({
                data: {
                    id,
                    tenant_id: data.tenant_id,
                    code,
            ...
            this.logger.error('客室ランク作成エラー', error);
            throw error;
        }
    }
    /**
     * 客室ランク一覧取得
     */
    static async getRoomGrades(tenantId) {
        try {
            this.logger.info('客室ランク一覧取得開始', { tenant_id: tenantId });
            const roomGrades = await prisma_1.hotelDb.getAdapter().room_grades.findMany({
                where: {
                    tenant_id: tenantId
                },
                orderBy: [
            ...
            this.logger.error('客室ランク一覧取得エラー', error);
            throw error;
        }
    }
    /**
     * 客室ランク詳細取得
     */
    static async getRoomGradeById(id, tenantId) {
        try {
            this.logger.info('客室ランク詳細取得開始', { id, tenant_id: tenantId });
            const roomGrade = await prisma_1.hotelDb.getAdapter().room_grades.findFirst({
                where: {
                    id,
                    tenant_id: tenantId
                }
            ...
            this.logger.error('客室ランク詳細取得エラー', error);
            throw error;
        }
    }
    /**
     * 客室ランク更新
     */
    static async updateRoomGrade(id, tenantId, data) {
        try {
            ...
            if (data.name && data.name !== existingGrade.name) {
                const duplicateGrade = await prisma_1.hotelDb.getAdapter().room_grades.findFirst({
                    where: {
                        tenant_id: tenantId,
                        name: data.name,
                        id: { not: id }
            ...
            const updatedGrade = await prisma_1.hotelDb.getAdapter().room_grades.update({
                where: { id },
                data: {
            ...
            this.logger.error('客室ランク更新エラー', error);
            throw error;
        }
    }
    /**
     * 客室ランク削除（物理削除）
     */
    static async deleteRoomGrade(id, tenantId, deletedBy) {
        try {
            ...
            // 物理削除
            await prisma_1.hotelDb.getAdapter().room_grades.delete({
                where: { id }
            });
            ...
            this.logger.error('客室ランク削除エラー', error);
            throw error;
        }
    }
}
exports.RoomGradeService = RoomGradeService;
