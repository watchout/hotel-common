...
exports.ResponseNodeRepository = void 0;
 ...
 * データベースとのやり取りを担当
 */
class ResponseNodeRepository {
    /**
     * ノード詳細を取得
     */
    async findNodeById(nodeId, language) {
        const node = await index_1.hotelDb.getAdapter().responseNode.findUnique({
            where: { id: nodeId },
            include: {
     ...
     * 子ノード一覧を取得
     */
    async findChildNodes(parentId, language) {
        const nodes = await index_1.hotelDb.getAdapter().responseNode.findMany({
            where: { parentId },
            select: {
                id: true,
                treeId: true,
    ...
    async createNode(data) {
        return index_1.hotelDb.getAdapter().responseNode.create({
            data: {
                id: (0, uuid_1.v4)(),
                treeId: data.treeId,
                // @ts-ignore - 型定義が不完全
    ...
    async createNodeTranslation(data) {
        return index_1.hotelDb.getAdapter().responseNodeTranslation.create({
            data: {
                id: (0, uuid_1.v4)(),
                nodeId: data.nodeId,
                // @ts-ignore - 型定義が不完全
    ...
    async updateNode(nodeId, data) {
        return index_1.hotelDb.getAdapter().responseNode.update({
            where: { id: nodeId },
            data
        });
    }
     ...
     * ノード翻訳を更新
     */
    async updateNodeTranslation(nodeId, language, data) {
        return index_1.hotelDb.getAdapter().responseNodeTranslation.upsert({
            where: {
                nodeId_language: {
                    nodeId,
                    language
...
exports.ResponseNodeRepository = ResponseNodeRepository;
