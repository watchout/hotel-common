...
class CursorAgentBridge {
    ...
    async detectProjectContext() {
        try {
            ...
            if (await this.fileExists(packageJsonPath)) {
                ...
                const name = packageJson.name || path_1.default.basename(workspaceRoot);
     ...
     * agentウィンドウからの入力を処理
     */
    async processAgentInput(userInput) {
        ...
        try {
            ...
            const finalResponse = await this.generateOptimizedResponse(userInput, ragResults, guardrailsResults, tokenOptimization);
...
`;
    ...
    determineTaskType(query) {
        if (query.includes('UI') || query.includes('デザイン'))
            return 'design';
        if (query.includes('エラー') || query.includes('修正'))
            return 'debug';
...
exports.CursorAgentBridge = CursorAgentBridge;
